...
GraphicNetwork::GraphicNetwork(const std::string& file, unsigned int timeout, const double& learningRate) :
    Network(file, learningRate) {
    ...
    for (const Vector& activation : activations) {
        ...
        if (layerSize > maxSize) maxSize = layerSize;
    }
    ...
    if (xSpacing > ySpacing) nodeRadius = ySpacing * 0.25;
    else nodeRadius = xSpacing * 0.25;
    if (nodeRadius > 0.05) nodeRadius = 0.05;
    ...
    for (size_t l(0); l < numberOfNeuronLayers; ++l) {
        ...
        for (size_t j(0); j < layerSize; ++j) {
            ...
            for (size_t k(0); k < prevLayerSize; ++k) {
                ...
                if (weight >= 0) {
                    if (maxWeight > 0) cr->set_source_rgb(0, weight / maxWeight, 0);
                    else cr->set_source_rgb(0, 0, 0);
                } else {
                    if (minWeight < 0) cr->set_source_rgb(weight / minWeight, 0, 0);
                    else cr->set_source_rgb(0, 0, 0);
                }
            ...
            if (bias >= 0) {
                if (maxWeight > 0) cr->set_source_rgb(0, bias / maxWeight, 0);
                else cr->set_source_rgb(0, 0, 0);
            } else {
                if (minWeight < 0) cr->set_source_rgb(bias / minWeight, 0, 0);
                else cr->set_source_rgb(0, 0, 0);
            }
            cr->move_to(nodeCoordinates[l+1][j].x, nodeCoordinates[l+1][j].y);
...
const std::vector<double> x({0, 0, 1, 1, 0.5, 0.5, 1  , 0  , 0.5});
const std::vector<double> y({0, 1, 0, 1, 0  , 1  , 0.5, 0.5, 0.5});
const std::vector<double> z({1, 1, 1, 1, 0  , 0  , 0  , 0  , 0  });

void GraphicNetwork::trainOnSample() {
    size_t index(random() % x.size());
    Vector _x({x[index], y[index]});
    Vector _y({z[index]});
    train(_x, _y);
    updateMaxWeights();
}
